...
var CPacket = (function () {
    ...
    function CPacket() {
        ...
        this.request = 0;
        this.encrypt = 0;
        this.encryptoriginlen = 0;
        this.compress = 0;
        this.compressoriginlen = 0;
        this.raw_data = new egret.ByteArray();
        //this.zips = new zip();
    }
    CPacket.prototype.ToBytes = function (buff) {
    ...
    CPacket.prototype.FromBytes = function (buff) {
        if (buff.bytesAvailable >= 4) {
            var pos = buff.position;
            var msg_size = buff.readInt();
            if (msg_size < 0 || msg_size > 0xffff) {
                throw ("msg size error!");
            ...
            //console.log("tmp:"+tmp)
            var key = new egret.ByteArray();
            for (var i = 0; i < 16 /*KEY.length*/; i++)
                key.writeByte(this.KEY[i]);
            ...
            if (this.encrypt != 0) {
                if (this.raw_data != null) {
                    var raw_data2 = new egret.ByteArray();
                    var aes2 = new rijndael();
            ...
            if (this.compress != 0) {
                if (this.raw_data != null)
                    def.uncompress(this.raw_data);
            }
            if (this.raw_data != null)
                this.raw_data.position = 0;
            //            console.log("===========================================");
            //            for(var jj:number =0 ;jj<this.raw_data.length;jj++)
            //            {
            //                this.raw_data.position = jj;
    ...
    CPacket.writeBinInt = function (b, value) {
...
__reflect(CPacket.prototype, "CPacket");
//# sourceMappingURL=CPacket.js.map
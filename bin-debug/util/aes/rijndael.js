...
var rijndael = (function () {
    function rijndael() {
        //Key schedule arrays
        this.Nr = 0; //Number of rounds [For 128 bit block, Nr = {10, 12, 14} for 128, 192, 256 bit cipher key]
        rijndael.fkey = new Array(60);
        rijndael.rkey = new Array(60);
    }
    ...
    rijndael.prototype.CreateDecryptionKeys = function (Nb) {
        if (Nb === void 0) { Nb = 0; }
        var i = 0;
        var j = Nb * this.Nr;
        var arr = new Array(4);
        ...
        for (var k = 0; k <= this.Nr; k++, i += Nb, j -= Nb) {
            for (var l = 0; l < Nb; l++)
    ...
    rijndael.prototype.SetCipherKey = function (key) {
        //128 bit block size
        var _key = new egret.ByteArray();
        _key.endian = egret.Endian.LITTLE_ENDIAN;
        _key.writeBytes(key, 0, Math.min(key.length, rijndael.KEYLENGTH));
        while (_key.length < rijndael.KEYLENGTH)
            _key.writeByte(0);
        _key.position = 0;
    ...
    rijndael.prototype.BlockEncrypt = function (plaintext, ciphertext) {
        ...
        var t = [p[0] ^ rijndael.fkey[0], p[1] ^ rijndael.fkey[1], p[2] ^ rijndael.fkey[2], p[3] ^ rijndael.fkey[3]];
        for (var i = 1, k = 4; i < this.Nr; i++, k += 4) {
            s.position = 0;
            ...
            s.position = 0;
            for (var z = 0; z < rijndael.BLOCKSIZE; z++)
    ...
    //plaintext and ciphertext can be the same array.
    rijndael.prototype.BlockDecrypt = function (ciphertext, plaintext) {
        var arr = new Array(16);
        var s = new egret.ByteArray();
        s.endian = egret.Endian.LITTLE_ENDIAN;
        ...
        var t = [c[0] ^ rijndael.rkey[0], c[1] ^ rijndael.rkey[1], c[2] ^ rijndael.rkey[2], c[3] ^ rijndael.rkey[3]];
        for (var i = 1, k = 4; i < this.Nr; i++, k += 4) {
            s.position = 0;
            //            s.writeUnsignedInt(t[0]);
            //            s.writeUnsignedInt(t[1]);
    ...
    rijndael.prototype.Encrypt = function (password, plaintext, ciphertext) {
        var cipherlen = plaintext.length + (rijndael.BLOCKSIZE - plaintext.length % rijndael.BLOCKSIZE) % rijndael.BLOCKSIZE;
        this.SetCipherKey(password);
        var plaintext_block = new egret.ByteArray();
        var ciphertext_block = new egret.ByteArray();
        var i = 0;
        ...
        if (i < plaintext.length) {
            ...
            while (plaintext_block.length < rijndael.BLOCKSIZE)
                plaintext_block.writeByte(0);
            this.BlockEncrypt(plaintext_block, ciphertext_block);
            ciphertext.writeBytes(ciphertext_block, 0, rijndael.BLOCKSIZE);
        }
        return cipherlen;
    };
    rijndael.prototype.Decrypt = function (password, ciphertext, plaintext) {
        if (ciphertext.length % rijndael.BLOCKSIZE != 0)
            throw new Error();
        this.SetCipherKey(password);
        var plaintext_block = new egret.ByteArray();
        var ciphertext_block = new egret.ByteArray();
        for (var i = 0; i < ciphertext.length; i += rijndael.BLOCKSIZE) {
            plaintext_block.position = 0;
            ciphertext_block.position = 0;
    ...
    rijndael.KEYLENGTH = 16;
    rijndael.BLOCKSIZE = 16;
    return rijndael;
}());
__reflect(rijndael.prototype, "rijndael");
//# sourceMappingURL=rijndael.js.map